...
def detect(gray, frame): # We create a function that takes as input the image in black and white (gray) and the original image (frame),
#and that will return the same image with the detector rectangles. 
    ...
    for (x, y, w, h) in faces: # For each detected face:
        ...
        eyes = eye_cascade.detectMultiScale(roi_gray, 1.1, 5) # We apply the detectMultiScale method to locate one or several eyes in the image.
